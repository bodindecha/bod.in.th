<?php
	...
	if (isset($_GET['path'])) {
		...
		if (file_exists($pifp)) {
			...
			$is_forced = ((isset($_GET['token']) && $_GET['token']==$_SESSION['var-image_token']) || (isset($_GET['download']) && $_GET['download']==strval(time())))
				&& count($_GET)==2;
			$name = basename($path);
			...
			if ((($hrfr=="" || explode("?", explode("//", $hrfr)[1])[0]==$_SERVER['SERVER_NAME'].$dir.$name) && !$is_forced) || $_SERVER['PHP_SELF']=="/resource/images/viewer") $nf = false;
			else {
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>';
		...
		echo '<style type="text/css">
			html body main {
				min-height: 100% !important; height: var(--h) !important;
				overflow: hidden;
			}
			...
			html body main div.container div.wrapper span {
				background-image: url("'.$dir.$name.'?token='.$_SESSION['var-image_token'].'"); background-size: contain;
		...
		<script type="text/javascript">
			...
			function download(e) {
				...
				if (cooldownload.t==null) {
					...
					$(adlder).attr("href", "'.$dir.$name.'?download="+(Math.round(Date.now()/1000)+0).toString()); adlder.click(); $(adlder).removeAttr("href");
	...
	<body class="nohbar">
		<main>
			<div class="container">
				<div class="wrapper">
					<div><img src="'.$dir.$name.'?token='.$_SESSION['var-image_token'].'"></div>
					'.($type=="svg" ? '<svg src="'.$dir.$name.'?token='.$_SESSION['var-image_token'].'"></svg>' : '<span></span>').'
				</div>
			</div>
			<div class="controller">
				...
				<div class="bar"><ul>
					...
					<span></span>
					<li>
						<a href="javascript:window.print()"><i class="material-icons">print</i><span data-title="Print (ctrl+P)"></span></a>
						<a href="javascript:download()"><i class="material-icons">download</i><span data-title="Download (ctrl+D)"></span></a>
						<a class="adlder" download="'.$name.'" style="display: none;"></a>
					</li>
				</ul></div>
			</div>
		</main>';
		...
		echo '<script type="text/javascript">var s=new XMLHttpRequest;s.open("GET","/error/900",!0),s.responseType="text",s.onload=function(){document.querySelector("html").innerHTML=this.responseText},s.send();</script>';
	}
?>